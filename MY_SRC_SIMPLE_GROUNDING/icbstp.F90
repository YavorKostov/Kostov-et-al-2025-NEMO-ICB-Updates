...
CONTAINS
   ...
   SUBROUTINE icb_stp( ktin, Kmm )
      ...
      DO indsubsteps = 1,numsubsteps
      kt = (ktin -1)*numsubsteps + indsubsteps
      nktberg = kt
      !
...
!      IF( nn_test_icebergs < 0 .OR. ln_use_calving ) THEN !* read calving data
 ...
 9100 FORMAT('kt= ',i8, ' day= ',i8,' secs=',i8)
      ...
      !
      !                                   !* For each berg, record trajectory
      !                                   (when needed)
      ll_sample_traj = .FALSE.
      IF( nn_sample_rate > 0 .AND. MOD(ktin-1,nn_sample_rate) == 0 ) ll_sample_traj = .TRUE.
      IF( ll_sample_traj .AND. ASSOCIATED(first_berg) )   CALL icb_trj_write( ktin )

      !                                   !* Gridded diagnostics
      !                                   !  To get these iom_put's and those
      !                                   preceding to actually do something
      !                                   !  use key_xios in cpp file and create
      !                                   content for XML file
      !
      ...
      IF( nn_verbose_level >= 2 )   CALL icb_utl_print( 'icb_stp, status', ktin )
      !
      !                                   !* Diagnose budgets
      ll_budget = .FALSE.
      IF( nn_verbose_write > 0 .AND. MOD(ktin-1,nn_verbose_write) == 0 ) ll_budget = ln_bergdia
      CALL icb_dia( ll_budget )
      !
      IF( lrst_oce ) THEN    !* restart
         CALL icb_rst_write( ktin )
