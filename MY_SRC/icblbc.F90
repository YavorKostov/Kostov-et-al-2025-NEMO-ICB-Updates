...
!$AGRIF_END_DO_NOT_TREAT
   ...
   INTEGER, PARAMETER, PRIVATE ::   jp_delta_buf = 26             ! Size by which to increment buffers
...
CONTAINS
   ...
   SUBROUTINE icb_lbc_mpp()
      ...
      DO i = 1, ibergs_rcvd_from_e
         IF( nn_verbose_level >= 4 ) THEN
            WRITE(numicb,*) 'bergstep ',nktberg,' unpacking berg ',INT(ibuffer_e%data(17,i)),' from east'
            CALL FLUSH( numicb )
         ENDIF
      ...
      DO i = 1, ibergs_rcvd_from_w
         IF( nn_verbose_level >= 4 ) THEN
            WRITE(numicb,*) 'bergstep ',nktberg,' unpacking berg ',INT(ibuffer_w%data(17,i)),' from west'
            CALL FLUSH( numicb )
         ENDIF
      ...
      DO i = 1, ibergs_rcvd_from_n
         IF( nn_verbose_level >= 4 ) THEN
            WRITE(numicb,*) 'bergstep ',nktberg,' unpacking berg ',INT(ibuffer_n%data(17,i)),' from north'
            CALL FLUSH( numicb )
         ENDIF
      ...
      DO i = 1, ibergs_rcvd_from_s
         IF( nn_verbose_level >= 4 ) THEN
            WRITE(numicb,*) 'bergstep ',nktberg,' unpacking berg ',INT(ibuffer_s%data(17,i)),' from south'
            CALL FLUSH( numicb )
         ENDIF
  ...
  1000      FORMAT(a,i5,a,i4)
      ...
      DO jn = 1, jpni
         IF( nicbfldproc(jn) /= -1 ) THEN
            ...
            DO jk = 1, ibergs_to_rcv
               IF( nn_verbose_level >= 4 ) THEN
                  WRITE(numicb,*) 'bergstep ',nktberg,' unpacking berg ',INT(ibuffer_f%data(17,jk)),' from north fold'
                  CALL flush( numicb )
               ENDIF
   ...
   SUBROUTINE icb_pack_into_buffer( berg, pbuff, kb )
      ...
      pbuff%data(16,kb) = berg%current_point%angleTopolagr
   ...
   SUBROUTINE icb_unpack_from_buffer(first, pbuff, kb)
      ...
      pt%angleTopolagr  =      pbuff%data(16,kb)
      DO ik = 1, nkounts
