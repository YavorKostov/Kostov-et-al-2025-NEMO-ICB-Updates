...
CONTAINS
   ...
   SUBROUTINE icb_utl_copy( Kmm )
      ...
      REAL(wp), PARAMETER ::  MaxPressureGradient = 1.E-4_wp   ! max allowed pressure gradient
      !
...
!              zcoef1 = zcoef0 * e3w(ji,jj,jk,Kmm)  ! scaling factor - includes
       ...
       ! Exchange data
         DO jk = 1,jpk
            ...
            ztmp(1:jpi,1:jpj) = zhpi(:,:,jk)
            CALL lbc_lnk_icb( 'icbutl', ztmp, 'U', -1._wp, 1, 1 )
            zhpi_e(1:jpi,1:jpj,jk) = ztmp(:,:)

            ztmp(1:jpi,1:jpj) = zhpj(:,:,jk)
            CALL lbc_lnk_icb( 'icbutl', ztmp, 'V', -1._wp, 1, 1 )
            zhpj_e(1:jpi,1:jpj,jk) = ztmp(:,:)
   ...
   SUBROUTINE icb_utl_interp( pi, pj, pe1 , pssu, pui, pua, pssh_i,         &
      ...
      IF ( PRESENT(izhpi ) .OR. PRESENT(izhpj) ) THEN
         ...
         zw1d(1,:) = zwU(1) ; zw1d(2,:) = zwU(2) ; zw1d(3,:) = zwU(3) ; zw1d(4,:) = zwU(4) ;
         zw1d(1,:) = umask(iiU  ,ijU  ,:) * zwU(1) * zmskU(1)
         zw1d(2,:) = umask(iiU+1,ijU  ,:) * zwU(2) * zmskU(2)
         zw1d(3,:) = umask(iiU  ,ijU+1,:) * zwU(3) * zmskU(3)
         zw1d(4,:) = umask(iiU+1,ijU+1,:) * zwU(4) * zmskU(4)
         izhpi(:) = icb_utl_bilin_h( zhpi_e , iiU, ijU, zw1d )


         zw1d(1,:) = zwV(1) ; zw1d(2,:) = zwV(2) ; zw1d(3,:) = zwV(3) ; zw1d(4,:) = zwV(4) ;
         zw1d(1,:) = vmask(iiV  ,ijV  ,:) * zwV(1) * zmskV(1)
         zw1d(2,:) = vmask(iiV+1,ijV  ,:) * zwV(2) * zmskV(2)
         zw1d(3,:) = vmask(iiV  ,ijV+1,:) * zwV(3) * zmskV(3)
         zw1d(4,:) = vmask(iiV+1,ijV+1,:) * zwV(4) * zmskV(4)
         izhpj(:) = icb_utl_bilin_h( zhpj_e , iiV, ijV, zw1d )
